...
public class PostgreCarDao extends PostgreEntityDao<Car> implements CarDao<Car> {
	...
	public Car get(Connection conn, Long id) {
		...
		try {
			...
			if (rs.next()) {
				Status carStatus = pCarStatusDao.get(conn, Long.valueOf(rs.getString(STATUS_ID)));
				car = new Car(Long.valueOf(rs.getString(ID)),
						rs.getString(MARK),
						rs.getString(MODEL),
						Double.valueOf(rs.getString(CAR_COST)),
						carStatus);
	...
	public List<Car> getAll(Connection conn) {
		...
		Car car = null;
		try {
			PreparedStatement pst = conn.prepareStatement(String.format(GET_ALL, getTableName()));
			ResultSet rs = pst.executeQuery();
